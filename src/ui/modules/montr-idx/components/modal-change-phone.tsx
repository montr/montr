...
export class ModalChangePhone extends React.Component<Props, State> {
	...
	fetchData = async () => {
		...
		const dataView = await this.metadataService.load(Views.formChangePhone);
	...
	handleSubmit = async (values: ProfileModel): Promise<ApiResult> => {
		...
		const result = await this.profileService.changePhone(values);
	...
	render = () => {
		...
		return (
			<Translation ns={Locale.Namespace}>
				{(t) => <Modal open={!loading} title={t("page.changePhone.title") as string}
